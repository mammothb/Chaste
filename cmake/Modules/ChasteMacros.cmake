...
macro(Chaste_ADD_TEST _testTargetName _filename)
    ...
    string(REGEX MATCH "^.*SipParallel$" foundSipParallel ${_testTargetName})
    if (foundSipParallel)
        set(parallel ON)
        string(REGEX REPLACE "(SipParallel)$" "" _testname "${_testTargetName}")
    else()
        set(parallel OFF)
        set(_testname ${_testTargetName})
    endif()
    ...
    foreach(type ${TestPackTypes})
        if(EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/${type}TestPack.txt")
            ...
            foreach(filename ${testpack})
                ...
                if (${type} STREQUAL "SipParallel")
                    set(testTargetName ${testTargetName}SipParallel)
                    set(parallel ON)
                endif()
